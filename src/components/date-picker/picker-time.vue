<template>
  <div class="picker-date">
    ...
    <div class="data-list" ref="pickerDate">
      <li v-for="(item,key) in ymdData" :id="key">
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      rangeDate02 = [Number(rangeDate02[0]) + 1, 12]
    ...
    methods: {
      initData (rangDate) { /* 初始化日期选择面板 */
        this.defaultTime = this.defaultDate.replaceDate()
        this.scrollDate = `${new Date(this.defaultTime).getFullYear()}-${new Date(this.defaultTime).getMonth() + 1}`
        // 循环年
      ...
      datePicker (yymm, item) { /* 选中日期处理 */
        ...
        let defaultTime = this.defaultDate.replaceDate()
        this.scrollDate = `${new Date(defaultTime).getFullYear()}-${new Date(defaultTime).getMonth() + 1}`
        this.checkPicker(this.defaultDate)
      },
      getDates (yy, mm) { /* 加工日数据(添加控制参数) */
        let monArr = []
        let week = new Date(`${yy}-${mm}-1`.replaceDate()).getDay()
        let days = new Date(yy, mm, 0).getDate()
        let defaultTime = new Date(this.defaultTime).getTime()
        for (let i = week, day = 1; i < (week + days > 35 ? 42 : 35); i++) {
          if (i < week + days) {
            ...
            monArr[i] = {
              ...
              isActiveNow: thisTime === this.nowTime // 今天的状态
      ...
      setScorll () {
        let scrollEle = document.getElementById(this.scrollDate)
